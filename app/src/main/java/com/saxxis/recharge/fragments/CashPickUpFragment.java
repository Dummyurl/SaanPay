...
import com.saxxis.recharge.activities.payment.LocationPickActivity;
...
import static com.saxxis.recharge.R.id.tipl_deposit_amount;
import static com.saxxis.recharge.R.id.tipl_deposit_emailadd;
import static com.saxxis.recharge.R.id.tipl_deposit_houseno;
import static com.saxxis.recharge.R.id.tipl_deposit_landmark;
import static com.saxxis.recharge.R.id.tipl_deposit_mobile;
import static com.saxxis.recharge.R.id.tipl_deposit_name;
import static com.saxxis.recharge.activities.payment.LocationPickActivity.MY_PERMISSIONS_REQUEST_LOCATION;
...
//    TextView txt100;
    @BindView(tipl_deposit_name)
    TextInputLayout tiplName;
    @BindView(tipl_deposit_mobile)
    TextInputLayout tiplMobile;
    @BindView(tipl_deposit_emailadd)
    TextInputLayout tiplEmail;
    @BindView(tipl_deposit_amount)
    TextInputLayout tiplAmount;
    @BindView(tipl_deposit_houseno)
    TextInputLayout tiplHouseNo;
    @BindView(tipl_deposit_landmark)
    TextInputLayout tiplLandMArk;
...
//        refreshWallet();
    ...
    private boolean isValidAllFields(String name, String mobile, String email, String amount, String address, String landmark, String houseno) {
            if (name.isEmpty()){
                tiplName.setError("Please Enter Name");
                return false;
            }
            ...
            if (mobile.length()<10){
                tiplMobile.setError("Please enter 10 digit mobile number");
                return false;
            }
            if (mobile.length()==10){
                tiplMobile.setError(null);
            }

            if (TextUtils.isEmpty(email)) {
                tiplEmail.setError("Please Enter Email");
                return false;
            } else {
            ...
            if (amount.isEmpty()){
                tiplAmount.setError("Please Enter Amount");
                return false;
            }
            if (!amount.isEmpty()){
                tiplAmount.setError("");
       ...
       if (address.isEmpty()){
        ...
        if (landmark.isEmpty()){
            tiplLandMArk.setError("Please Enter Land Mark");
            return false;
        }
        if (!landmark.isEmpty()){
            tiplLandMArk.setError("");
        }

        if (houseno.isEmpty()){
            tiplHouseNo.setError("Please Enter House Number");
            return false;
        }

        if (!houseno.isEmpty()){
            tiplHouseNo.setError("");
        }
